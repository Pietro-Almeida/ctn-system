...
export default function CreateNewsPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSubmitting(true)
    try {
      const created = await createNews({ titulo: titulo.trim(), conteudo: conteudo.trim(), categoria }, token)
    ...
    <main className="create-news-page">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <aside className="create-news-preview">
          ...
          <div className="create-news-art" aria-hidden="true"><i /><span /></div>
        ...
        <footer><Link to={`${base}/jornal`}>Cancelar</Link><button type="submit" disabled={submitting}>{submitting ? 'Publicando...' : 'Publicar no Jornal'}</button></footer>
      </form>
    </main>
  )
}
